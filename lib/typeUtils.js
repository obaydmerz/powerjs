...
function generateDirectObj(obj, dll, d, item, exec) {
  const { isMethod } = getItemDefData(
    dll.defenition[item]
  );
  if (isMethod) {
    ...
    Object.defineProperty(obj[d], item, {
      async get() {
        return exec(`[DLL.${d}]::${item}`);
      },
      async set(v) {
        await exec(`[DLL.${d}]::${item} = ${encodePS(v)}`);
      },
    });
  }
}
...
export function generateDllDirectObj(dlls, exec) {
  ...
  for (const d in dlls) {
    if (Object.hasOwnProperty.call(dlls, d)) {
      ...
      for (const item in dll.defenition) {
        if (Object.hasOwnProperty.call(dll.defenition, item)) {
          generateDirectObj(obj, dll, d, item, exec)
        }
      }
    }
  }
